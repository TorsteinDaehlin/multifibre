...
if S.multifibre.use_multifibre_muscles
    Fcetilde = [];
    for m = 1:N_muscles
        [Hilldiff(m), FT(m), Fce(m), Fpass(m), Fiso(m), vMmax(m,:), massM(m), Fcetilde_tmp] = ...
            ...
            N_fibre);
        Fcetilde = [Fcetilde; Fcetilde_tmp];
    end
else
    for m = 1:N_muscles
...
f_forceEquilibrium_FtildeState_all_tendon = ...
    ...
    dFTtilde,lMT,vMT,tension_SX},{Hilldiff,FT,Fce,Fpass,Fiso,vMmax,massM,Fcetilde},...
    {'a','FTtilde','dFTtilde','lMT','vMT','tension_SX'},...
    {'Hilldiff','FT','Fce','Fpass','Fiso','vMmax','massM','Fcetilde'});
