function [] = OCP_formulation_cycling(S,model_info,f_casadi)
% --------------------------------------------------------------------------
% OCP_formulation
...
if strcmp(S.solver.IG_selection,'quasi-random')
    guess = getGuess_QR_cycling(S, model_info, scaling, d);
else
    guess = getGuess_DI_cycling(S,model_info,scaling,d);
end
...
% For cycling, the pedal forces are added as state variables at mesh points
% and collocation points. TODO: MOVE TO MODEL INFO
n_FPedal = numel(model_info.ExtFunIO.input.Forces.pedal_force_r) + ...
    numel(model_info.ExtFunIO.input.Forces.pedal_force_l);

FPedal = opti.variable(n_FPedal, N+1);
opti.subject_to(bounds.FPedal.lower'*ones(1,N+1) < FPedal < ...
    bounds.FPedal.upper'*ones(1,N+1));
opti.set_initial(FPedal, guess.FPedal');

FPedal_col = opti.variable(n_FPedal, d*N);
opti.subject_to(bounds.FPedal.lower'*ones(1,d*N) < FPedal_col < ...
...
for j=1:d
    ...
    [Hilldiffj,FTj,Fcej,Fpassj,Fisoj] = ...
        f_casadi.forceEquilibrium_FtildeState_all_tendon(reshape(akj(:,j+1), [NFibre NMuscle])',...
    ...
    if strcmp(S.metabolicE.model,'Bhargava2004')
        ...
        [e_totj,~,~,~,~,~] = f_casadi.getMetabolicEnergySmooth2004all(...
            reshape(akj(:,j+1), [NFibre NMuscle])',reshape(akj(:,j+1), [NFibre NMuscle])', ...
            lMtildej,vMj,Fcej,Fpassj,MuscleMass',pctsts,Fisoj,model_info.mass, ...
            S.metabolicE.tanh_b);
    elseif strcmp(S.metabolicE.model,'MinettiAlexander')
        e_totj = ...
            f_casadi.getMetabolicEnergyMinettiAlexander_multifibre(...
            reshape(akj(:,j+1), [NFibre NMuscle])', vMj);
    else
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Crank velocity constraints
    crank_r = Tj(model_info.ExtFunIO.position.pedal_r);
    crank_l = Tj(model_info.ExtFunIO.position.pedal_l);
    omega_crank_r = cross(crank_r, Tj(model_info.ExtFunIO.velocity.pedal_r)) / ...
        norm(crank_r)^2;
...
for k=1:N
    ...
    FPedalkj = [FPedal(:,k), FPedal_col(:,(k-1)*d+1:k*d)];
    ...
    opti.subject_to(FPedal(:,k+1) == FPedalkj*D);
...
if strcmp(S.misc.gaitmotion_type,'HalfGaitCycle')
    ...
    % Muscle activations
    opti.subject_to(a(model_info.ExtFunIO.symQs.MActInvA,end) - a(model_info.ExtFunIO.symQs.MActInvB,1) == 0);
...
FPedal_opt = reshape(w_opt(starti:starti+n_FPedal*(N+1)-1),n_FPedal,N+1)';
starti = starti + n_FPedal*(N+1);
FPedal_col_opt = reshape(w_opt(starti:starti+n_FPedal*(d*N)-1),n_FPedal,d*N)';
starti = starti + n_FPedal*(d*N);
...
FPedal_mesh_col_opt=zeros(N*(d+1)+1,n_FPedal);
FPedal_mesh_col_opt(1:(d+1):end,:)= FPedal_opt;
if nq.torqAct > 0
...
for k=1:N
    ...
    FPedal_mesh_col_opt(rangei,:) = FPedal_col_opt(rangebi,:);
end
...
FPedal_opt_unsc = FPedal_opt(1:end-1,:).*repmat(...
    scaling.FPedal,size(FPedal_opt(1:end-1,:),1),1);
...
FPedal_col_opt_unsc = FPedal_col_opt.*repmat(scaling.FPedal,size(FPedal_col_opt,1),1);
...
for k=1:N
    for j=1:d
        ...
        [~,~,Fce_opt_all,Fpass_opt_all,Fiso_opt_all] = ...
            ...
            reshape(a_col_opt_unsc(count,:)',NFibre,[])',FTtilde_col_opt_unsc(count,:)',...
        ...
        if strcmp(S.metabolicE.model,'Bhargava2004')
            ...
            reshape(a_col_opt_unsc(count,:)',NFibre,[])',reshape(a_col_opt_unsc(count,:)',NFibre,[])',...
        ...
        elseif strcmp(S.metabolicE.model,'MinettiAlexander')
            e_tot_all = ...
                f_casadi.getMetabolicEnergyMinettiAlexander_multifibre(...
                reshape(a_col_opt_unsc(count,:)',NFibre,[])', full(vM_opt_all));
        else
            error('No energy model selected');
        end
        ...
        J_opt = J_opt + (...
            ...
            W.a*B(j+1)          *(f_casadi.J_act_exp(a_col_opt(count,:), W.a_exp))*h_opt + ...
            ...
            W.slack_ctrl*B(j+1) *(f_casadi.J_act(vA_opt(k,:)))*h_opt + ...
            W.slack_ctrl*B(j+1) *(f_casadi.J_muscles(dFTtilde_col_opt(count,:)))*h_opt);
            
        if nq.torqAct > 0
            J_opt = J_opt + (W.e_torqAct*B(j+1)      *(f_casadi.J_torq_act(e_a_opt(k,:)))*h_opt);

            Actu_cost = Actu_cost + W.e_torqAct*B(j+1)*(f_casadi.J_torq_act(e_a_opt(k,:)))*h_opt;
        end
        if nq.arms > 0
            J_opt = J_opt + (W.slack_ctrl*B(j+1) *(f_casadi.J_arms_dof(qdotdot_col_opt(count,model_info.ExtFunIO.jointi.armsi)))*h_opt);
        ...
        A_cost = A_cost + W.a*B(j+1)*...
            (f_casadi.J_act_exp(a_col_opt(count,:), W.a_exp))*h_opt;      
...
assertCost = abs(J_optf - (E_costf+A_costf + Arm_costf + ...
...
if strcmp(S.misc.gaitmotion_type,'HalfGaitCycle')
    ...
    a_opt_unsc(idx_2nd_half_GC,model_info.ExtFunIO.symQs.MActInvA) = a_opt_unsc(idx_2nd_half_GC,model_info.ExtFunIO.symQs.MActInvB);

    % Time derivatives of muscle activations
    vA_opt_unsc = [vA_opt_unsc(1:end,:); vA_opt_unsc(1:end,:)];
    vA_opt_unsc(idx_2nd_half_GC,model_info.ExtFunIO.symQs.MActInvA) = vA_opt_unsc(idx_2nd_half_GC,model_info.ExtFunIO.symQs.MActInvB);
    ...
    FPedal_opt_unsc = [FPedal_opt_unsc(1:end,:); FPedal_opt_unsc(1:end,:)];
    FPedal_opt_unsc(idx_2nd_half_GC,1:6) = FPedal_opt_unsc(idx_2nd_half_GC,[4 5 6 1 2 3]); %TODO: REMOVE HARDCODED VARIABLES
end
...
%% Gait cycle starts at right side pedal in the forward position
...
for i = 1:size(q_opt_unsc.rad,1)
    ...
    % Assign pedal forces
    F_ext_input([model_info.ExtFunIO.input.Forces.pedal_force_r ...
        model_info.ExtFunIO.input.Forces.pedal_force_l],1) = FPedal_opt_unsc(i,:);
    % Evaluate external function
    res = F(F_ext_input);
    Foutk_opt(i,:) = full(res);
end
pedal_r_opt = Foutk_opt(:,model_info.ExtFunIO.position.pedal_r);
[~, start_idx] = max(pedal_r_opt(:,1)); % The pedal is in the forward position when the x-coordinate is at its maximum value
idx_GC = [start_idx:size(q_opt_unsc.rad,1) 1:start_idx-1];
...
FPedal_GC = FPedal_opt_unsc(idx_GC, :);
if nq.torqAct > 0
